  ...
  return (
    <>
      <motion.div 
        ...
        className={`bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm transition-all ${available ? 'hover:shadow-2xl cursor-pointer' : 'opacity-60 grayscale-[0.5] cursor-not-allowed'}`}
      >
        ...
        <div className="p-6">
          ...
          <div className="space-y-2 mb-6 text-blue-600 font-medium text-xs">
            ...
            {metro && <div className="flex items-center gap-2 font-bold"><Footprints className="w-3.5 h-3.5" /> {metro}</div>}
          </div>
  ...
  return (
    <div className="font-sans selection:bg-blue-600 selection:text-white antialiased overflow-x-hidden text-slate-900 bg-white">
      ...
      <section className="pt-32 pb-24 max-w-6xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
        <Reveal>
          <h1 className="text-6xl md:text-7xl font-display font-black leading-[1] tracking-tighter mb-6 uppercase">Makaan. <br /><span className="text-slate-400 font-light italic">Study Smart.</span></h1>
          <p className="text-lg text-slate-500 font-light mb-8 max-w-md italic">The premier choice for student living in Al Barsha, Dubai.</p>
          <button onClick={() => document.getElementById('living').scrollIntoView({behavior:'smooth'})} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black text-lg shadow-2xl hover:bg-slate-900 transition tracking-tight uppercase">Explore Spaces</button>
        </Reveal>
      ...
      {/* Living Spaces with Slider and Local Images */}
      <section id="living" className="py-24 max-w-6xl mx-auto px-6">
        <Reveal>
          <h2 className="text-5xl font-display font-black mb-16 tracking-tighter uppercase italic">Living Spaces</h2>
          <div className="grid md:grid-cols-3 gap-10">
            ...
            />
            <RoomCard title="Premium Triple" price="3,000" location="10 mins" available={false} images={["https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&q=80&w=800"]} />
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <Reveal>
            <div className="text-center mb-16">
              ...
              <p className="text-blue-100 text-sm font-light uppercase tracking-widest">High-performance living for high-performance students.</p>
            </div>
      ...
      {/* Contact Form with New Dropdowns */}
      <section id="contact" className="py-24 max-w-6xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-start">
        <Reveal>
          <h2 className="text-7xl font-display font-black leading-[0.9] tracking-tighter mb-8 uppercase italic">Book <br /> Your Spot <span className="text-blue-600">.</span></h2>
          <div className="space-y-4 font-bold text-lg text-slate-600">
            <div className="flex items-center gap-4 uppercase tracking-widest"><Phone className="text-blue-600" /> +971 50 XXX XXXX</div>
            <div className="flex items-center gap-4 uppercase tracking-widest"><Mail className="text-blue-600" /> hello@makaan.ae</div>
          </div>
        </Reveal>
        <Reveal>
          <form onSubmit={handleSubmit} className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-50">
            <div className="grid grid-cols-2 gap-4 mb-4">
              <input name="firstName" placeholder="First Name" required className="p-4 rounded-2xl bg-slate-50 border-none outline-none focus:ring-2 ring-blue-100 text-sm font-bold" />
              <input name="lastName" placeholder="Last Name" required className="p-4 rounded-2xl bg-slate-50 border-none outline-none focus:ring-2 ring-blue-100 text-sm font-bold" />
            </div>
            <div className="grid grid-cols-2 gap-4 mb-4">
               <select name="gender" required className="p-4 rounded-2xl bg-slate-50 text-slate-400 text-sm font-bold"><option value="">Gender</option><option>Male</option><option>Female</option></select>
               <select name="institute" required className="p-4 rounded-2xl bg-slate-50 text-slate-400 text-sm font-bold"><option value="">Institute</option><option>Middlesex</option><option>Heriot-Watt</option><option>UOWD</option><option>Other</option></select>
            </div>
            <input name="email" type="email" placeholder="Email Address" required className="w-full mb-4 p-4 rounded-2xl bg-slate-50 border-none text-sm font-bold" />
            <button className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg hover:bg-blue-600 transition shadow-xl uppercase tracking-widest">Apply Now</button>
            {status && <p className="mt-4 text-center text-blue-600 font-black text-xs uppercase animate-pulse">{status}</p>}
          </form>
        </Reveal>
      </section>

      <footer className="py-12 border-t border-gray-100 text-center text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">
